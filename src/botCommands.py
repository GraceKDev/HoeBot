...
def setup(intents: Intents) -> commands.Bot:
	...
	async def completed(interaction: discord.Interaction, member:discord.Member = None):
		if member is None:
		...
		await interaction.response.send_message(content=pages[0], view=view, ephemeral=True)
